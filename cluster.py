...
class extract_label:
 ...
 ## 截断200个词版本
    def load_json_data_without_label_with_segmentation2(self, file):
        with open(file, 'r', encoding='utf-8') as f:
            result = []
            for line in f.readlines():
                dic = json.loads(line)
    ...
    def cluster_from_pretrain(self, filedir, select_titleids=None, online=False, dis_cutoff_ratio=None, pre_cut_off=None):
        ...
        docnums = density.shape[0]
        with open(os.path.join(filedir, 'index2id.json'), 'r', encoding='utf8') as f:
            index2id = json.load(f)
        id2index = {}
        for k, v in index2id.items():
            id2index[v] = int(k)
        index2id = {}
        for k, v in id2index.items():
            index2id[v] = k
        select_index = []
        if not select_titleids:
            select_index = list(range(docnums))
        else:
            for k in select_titleids:
                select_index.append(id2index[k])
        distance = distance[select_index]
        distance = distance[:, select_index]


        distance_sort = []
        docnums = distance.shape[0]
        for index_i in range(docnums):
            for index_j in range(index_i + 1, docnums):
                distance_sort.append(distance[index_i, index_j])
        distance_sort.sort()
        if dis_cutoff_ratio:
            self.dis_cutoff_ratio = dis_cutoff_ratio
        cut_off = distance_sort[int(len(distance_sort)*self.dis_cutoff_ratio)]
        if pre_cut_off:
            cut_off = pre_cut_off
        density = density[select_index]
        distance_higherDensity = distance_higherDensity[select_index]
        ...
        result = self.handle_and_filter_result(result)
        if not online:
            return result
        tmp = {}
    ...
    def title_df_max3(self,  title_words, stopwords, threshold=0.6):
            ...
            for k in cur_title:
                ...
                for j in range(start + 1, l):
                    if k in title_words[j]:
                        word_df[k] += 1
            start += 1
        t = sorted(word_df.items(), key=lambda x: x[1], reverse=True)
        labels = []
        for i in t:
            if  len(labels)<3 or i[1]>=threshold:
                labels.append(i[0])
            else:
                break
        return labels
    ...
    def handle_and_filter_result(self, result, threshold=4):
        tmp = {}
        index = 0
        for k in result:
            t = result[k]
            ...
            for i in t:
                ...
                origin_titles.append(''.join(self.title_segment[i]))
                segment_titles.append(self.title_segment[i])
            # labels = self.title_df(cluster_titles, self.stopwords)
            labels = self.title_df_max3(cluster_titles, self.stopwords)
            titleids = []
            for i in t:
            ...
            labels = set(labels)
            cur_label_nums = len(labels)
            span_label = {}
            for candidate in segment_titles:
                start_index = -1
                end_index = -1
                score = 0
                for can_index, candidate_word in enumerate(candidate):
                    if candidate_word in labels:
                        score += 1
                        if start_index == -1:
                            start_index = can_index
                            end_index = can_index
                        else:
                            end_index = can_index
                    if start_index>-1:
                        generate_title = candidate[start_index:end_index+1]
                        generate_title = ''.join(generate_title)
                        span_label[generate_title] = [min(score, cur_label_nums), -1*(end_index-start_index+1)]
            span_label = sorted(span_label.items(), key=lambda x:x[1], reverse=True)
            tmp[index]['span_label'] = span_label[0][0]
            index += 1
        return tmp
...
if __name__ == "__main__":
    ...
    # s.cal_distance_matrix_and_save(filedir='test')
    # ids = None
    t = s.cluster_from_pretrain(filedir='test', select_titleids=ids, dis_cutoff_ratio=0.0006, online=False,
                                pre_cut_off=0.98)
    # # print(len(t))
    # pprint(t)
    # ids = []
    # with open(os.path.join('test', 'index2id.json'), 'r', encoding='utf8') as f:
    #     index2id = json.load(f)
    # for k in index2id:
    #     if int(k)>=800:
    #         break
    #     ids.append(index2id[k])

    # for i in t:
    #     print(t['labels'])
    #  pre_cut_off=0.9999999999999997是原版本效果

    # s.cal_distance_matrix_and_save(filedir='test200')
    # ids = None
    # t = s.cluster_from_pretrain(filedir='test200', select_titleids=ids, dis_cutoff_ratio=0.0006, online=False)
    # # print(len(t))
    # pprint(t)
    # t = s.cluster_from_pretrain(filedir='test', select_titleids=ids, dis_cutoff_ratio=0.0006, online=False)
    # # print(len(t))
    ...
    # t = s.cluster_from_pretrain(filedir='test', select_titleids=ids, dis_cutoff_ratio=0.0006, online=True)
    # pprint(t)
    ...
    #     result = []
    #     for id in v:
    #         print(s.title_segment[id2index[id]])
    #     print('***'*8)
